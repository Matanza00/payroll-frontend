...
const TaxManagementUpdateForm = () => {
  const { id } = useParams();
  const navigate = useNavigate();
  const { showErrorToast, showSuccessToast } = useToast();
  const { data, isLoading: GetTaxManagementLoader } = useGetTaxQuery(id);
  const [formValues, setFormValues] = useState({
    payrollId: '',
    taxType: '',
    amount: '',
  });
  const [UpdateTaxManagement, { isLoading }] = useUpdateTaxMutation();
  ...
  const handleUpdate = async (e) => {
    e.preventDefault();
    try {
      await UpdateTaxManagement({
        id,
        ...formValues,
        amount: parseFloat(formValues.amount),
      }).unwrap();
      showSuccessToast('Tax Management Record Updated Successfully!');
      navigate(-1);
    } catch (err) {
      showErrorToast('An error occurred while updating tax management record');
    }
  };
  ...
  if (GetTaxManagementLoader) return <Loader />;

  return (
    <DefaultLayout>
      <div className="mx-auto max-w-600">
        <BreadcrumbNav
          pageName="Edit Tax Management"
...
export default TaxManagementUpdateForm;
